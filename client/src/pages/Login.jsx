...
export default function Login() {
	...
	const googleLogin = useGoogleLogin({
		...
		onError: (_errorResponse) => setError("Google login failed"),
	});
		...
		<AuthLayout title="Welcome back" subtitle="Log in to your account">
			...
			<div className="relative my-6">
				...
				<div className="relative flex justify-center text-sm">
					<span className="px-2 bg-(--color-surface) text-(--color-foreground) opacity-70">
			...
			<form onSubmit={handleSubmit} className="flex flex-col gap-5">
				...
				<div className="flex justify-end -mt-3">
					<Link
						to="/forgot-password"
						className="text-sm text-(--color-accent) hover:underline text-right"
					>
						Forgot password?
					</Link>
				</div>
				...
				<p className="text-center text-sm text-(--color-foreground) mt-4">
					Don't have an account?{" "}
					<Link to="/signup" className="text-(--color-accent) font-medium hover:underline">
						Sign up
					</Link>
				</p>
			</form>
