...
export async function wsSecurityMiddleware(req) {
	...
	try {
		...
		if (arcjetDecision.isDenied()) {
			const { code, reason } = arcjetDecision.reason.isRateLimit()
				? {
						code: 1013,
						reason: "Too Many Requests. Please try again after sometime.",
					}
		...
		return {
			success: false,
			code: 1011,
			reason: "Server security error. Please try again later.",
		};
	}
}
